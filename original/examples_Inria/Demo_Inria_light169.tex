...
%-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	%\setbeameroption{show notes on second screen=right} % Requires \usepackage{pgfpages} and pympress or pdfpc
		...
		% Define the "small words" that should stay lowercase (unless first or last)
		\TitlecaseExclusions{a an the and but or for nor on at to from by in of up with as if yet so}
...
%-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	\titlegraphic{
		...
		\includegraphics[height=0.08\paperheight]{\PathSO/UGE/logos/UGE-logo_white.pdf}
		\\[-90pt]
		\hspace{1.2689\paperwidth}\includegraphics[height=0.10562\paperheight]{\PathSO/Inria/logos/header_i4s-crop.pdf}
		\\[443pt]
	}
	...
	\renewcommand{\gothamInstituteLogoSquare}[1][4ex]{
		\includegraphics[height=#1-1.75ex]{\PathSO/UGE/logos/UGE-logo.pdf}
		\includegraphics[height=#1-1.75ex]{\PathSO/Inria/logos/inr_logo_rouge.pdf}
	}
